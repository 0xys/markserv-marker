...
- On comment creation, `registry.narrowRange` shrinks the UI-reported block range to the lines the selected text (`quote`) actually touches, and a `snapshot` of those lines is stored.
- `getThreads` recomputes `currentText`/`changed` per read; if the snapshot text still exists verbatim anywhere in the file, the lines merely shifted and `changed` stays false.
