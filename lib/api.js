...
const createApiHandler = deps => {
	...
	// The one endpoint that changes a served document. Takes the lines the
	// caller read, the text they held then, and what to put there instead;
	// registry.replaceLines refuses to write if that text has gone, unless
	// the caller says force. No notification is sent: the hot-reload watcher
	// sees the write and pushes a re-render on its own 150ms later, and a
	// second push would only rebuild every widget twice.
	const patchContent = async (req, res, [id]) => {
		const reg = registry.get(id)
		if (!reg) {
			return sendError(res, 404, 'not-registered', `No registration with id: ${id}`)
		}
		...
		if (reg.type !== 'file') {
			return sendError(res, 400, 'not-a-file', 'Content is only editable for file registrations')
		}

		const body = await readJsonBody(req)

		if (!Number.isInteger(body.lineStart) || body.lineStart < 1) {
			throw badRequest('invalid-line-start', '"lineStart" must be an integer of at least 1')
		}

		if (!Number.isInteger(body.lineEnd) || body.lineEnd < body.lineStart) {
			throw badRequest('invalid-line-end', '"lineEnd" must be an integer no smaller than "lineStart"')
		}

		if (typeof body.base !== 'string') {
			throw badRequest('missing-base', 'Provide "base": the text those lines held when they were read')
		}

		if (typeof body.text !== 'string') {
			throw badRequest('missing-text', 'Provide "text": what those lines should hold instead')
		}

		let written
		try {
			written = registry.replaceLines(reg.id, {
		...
		sendJson(res, 200, {id: reg.id, path: reg.path, ...written})
	}
	...
	const createComment = async (req, res, [id]) => {
		...
		if (body.quoteIndex !== undefined && body.quoteIndex !== null &&
			(!Number.isInteger(body.quoteIndex) || body.quoteIndex < 0)) {
			throw badRequest('invalid-quote-index',
				'"quoteIndex" must be a non-negative integer')
		}
	...
	const clearComments = (req, res, [id], searchParams) => {
		const reg = registry.get(id)
		if (!reg) {
			return sendError(res, 404, 'not-registered', `No registration with id: ${id}`)
		}
		...
		const deleted = registry.clearComments(reg.id, options)
		if (deleted > 0) {
			notify.commentChange(reg.id)
		}

		sendJson(res, 200, {fileId: reg.id, deleted})
	}
	...
	const routes = [
		...
		['PATCH', /^\/files\/([^/]+)\/content$/, patchContent],
		['GET', /^\/files\/([^/]+)\/comments$/, listComments],
		['POST', /^\/files\/([^/]+)\/comments$/, createComment],
		['DELETE', /^\/files\/([^/]+)\/comments$/, clearComments],
