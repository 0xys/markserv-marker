...
// Replace lines [lineStart, lineEnd] of a registered file with `text`, but
// only where `base` — the text those lines held when the caller read them —
// can still be found. This is the only place anything writes to a served
// document, and two things make that safe.
//
// It locates the range the way getThreads re-anchors a comment: the same lines
// if they still hold `base`, otherwise findSnapshotLines, so "the write is
// safe" and "the comments have not moved" are the same predicate. A caller
// whose base has genuinely gone gets a stale-content error carrying the
// current text, and has to say `force` to overwrite by line number instead.
//
// And it writes through a temp file in the same directory, renamed into place:
// getThreads and getContent read the file synchronously on every request, and
// a truncate-then-write window would let them see a half-written document —
// which getThreads swallows into "nothing changed" rather than reporting.
// Thrown when the base text is gone: the API turns it into a 409 carrying
// the current text, so the caller can show what is there now and decide.
const staleContent = (message, current) => {
	const error = new Error(message)
	error.status = 409
	error.code = 'stale-content'
	error.current = current
	return error
}

const replaceLines = (fileId, {lineStart, lineEnd, base, text, force}) => {
	const reg = registrations.get(fileId)
	if (!reg || reg.type !== 'file') {
		return null
	}

	// Splitting on newlines keeps a trailing '' for a file that ends in one.
	// Keeping it is what makes the line numbers here agree with getContent and
	// with every snapshot, and what preserves the final newline on the way out.
	const lines = fs.readFileSync(reg.path, 'utf8').split('\n')
	const at = {start: lineStart, end: Math.min(lineEnd, lines.length)}
	const current = lines.slice(at.start - 1, at.end).join('\n')

	if (current !== base) {
		const moved = findSnapshotLines(lines, base, lineStart)
		if (moved) {
			at.start = moved.lineStart
			at.end = moved.lineEnd
		} else if (!force) {
			throw staleContent(
				'Those lines no longer hold the text this edit was based on',
				current)
		}
	}

	// Follow the line endings already in the file, or every snapshot in it
	// stops matching verbatim and every thread reports itself as edited
	const crlf = lines.slice(at.start - 1, at.end).some(line => line.endsWith('\r'))
	const replacement = text.split('\n').map(line => {
		const bare = line.endsWith('\r') ? line.slice(0, -1) : line
		return crlf ? bare + '\r' : bare
	})

	const next = [...lines.slice(0, at.start - 1), ...replacement, ...lines.slice(at.end)]

	// Extension deliberately outside the hot-reload watcher's list, so the
	// temp file itself does not announce a change
	const temp = path.join(path.dirname(reg.path), '.' + path.basename(reg.path) + '.marker-tmp')
	fs.writeFileSync(temp, next.join('\n'))
	fs.renameSync(temp, reg.path)

	return {
		lineStart: at.start,
		lineEnd: at.start + replacement.length - 1,
		lines: next.length
	}
}
