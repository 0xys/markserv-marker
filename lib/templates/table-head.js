...
(function () {
	...
	function addShell(table) {
		...
		// A widget anchored to a header row rides in a row of its own inside
		// the thead, and copying that hands the page a second, fixed copy of a
		// comment form or an editor — one that answers to nothing, sits over
		// everything, and carries the first one's measured highlights at
		// coordinates that mean nothing where it landed
		var head = table.tHead.cloneNode(true);
		var widgets = head.querySelectorAll('[data-marker-ui]');
		for (var w = 0; w < widgets.length; w++) widgets[w].remove();
		clone.append(head);
	...
	function headCells(head) {
		return [].filter.call(head.querySelectorAll('th, td'), function (cell) {
			return !cell.closest('[data-marker-ui]');
		});
	}
	...
	function sync() {
		shells.forEach(function (entry) {
			...
			// Only the header's own cells: a widget's cell has no counterpart
			// in the copy, and counting it in would shift every width by one
			var real = headCells(entry.table.tHead);
