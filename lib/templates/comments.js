...
(function () {
	...
	function ensureSelectionButton() {
		...
		selectionButton = el('button', 'marker-select-btn');
		selectionButton.setAttribute('data-marker-ui', '');
		// The lines about to be commented on, worn the way the mermaid and diff
		// toggles wear their unresolved count: a translucent badge inside the
		// button, not a control of its own
		selectionLines = el('span', 'marker-select-lines');
		selectionButton.appendChild(document.createTextNode('💬 Comment'));
		selectionButton.appendChild(selectionLines);
	...
	function maybeShowSelectionButton() {
		...
		selectionLines.textContent = lineStart === lineEnd ?
	...
	function clearWidgets() {
		...
		var stale = document.querySelectorAll(
			'.marker-thread, .marker-thread-row, .marker-form, #marker-unanchored');
