...
(function () {
	...
	// Once now, and once more a tick later: the first is for the reader, the
	// second for whatever the browser does with the mouse gesture after the
	// handler returns — the form used to focus only from a setTimeout, and
	// that was the version known to leave the reader typing
	function focusSoon(options) {
	...
	// Focus for a textarea the reader asked for scrolls it into view, as
	// focus() always did: a form opened below the fold with the caret blinking
	// out of sight reads as a form with no caret. Focus given back after a
	// re-render does not — it is where they were typing, not where they were
	// reading, and every comments push used to drag the page back to it.
	function restoreFocus(options) {
		...
		var scroll = Boolean(options && options.scroll);
		if (document.activeElement === textarea) {
			// Already focused, so focus() would not move the page
			if (scroll && typeof textarea.scrollIntoView === 'function') {
		...
		textarea.focus({preventScroll: !scroll});
	...
	// Shift+Enter posts and plain Enter is the newline, unless the index page
	// has been told to swap them, in which case Enter posts and Shift+Enter is
	// the newline. Read at each keypress rather than kept, so the choice made
	// on that page — another tab, usually — needs no reload here.
	var SUBMIT_KEY = 'markserv-marker-submit-key';

	function plainEnterSubmits() {
		try {
			return localStorage.getItem(SUBMIT_KEY) === 'enter';
		} catch (_) {
			return false;
		}
	}

	// Whether this keydown is the one that submits. isComposing, and keyCode
	// 229 for the browsers that do not set it, guard the Enter that commits an
	// IME conversion candidate: in Japanese that Enter is part of typing a
	// word, and posting on it loses the sentence.
	function submitsOn(event) {
		if (event.key !== 'Enter') return false;
		if (event.isComposing || event.keyCode === 229) return false;
		return plainEnterSubmits() ? !event.shiftKey : event.shiftKey;
	}

	// The shortcut spelled out beside the buttons, and re-spelled when the
	// setting changes in another tab: a hint that disagrees with the keyboard
	// is worse than no hint
	function hintNode(verb) {
		var hint = el('span', 'marker-form-hint');
		hint.setAttribute('data-submit-verb', verb);
		hint.textContent = (plainEnterSubmits() ? 'Enter' : 'Shift+Enter') + ' to ' + verb;
		return hint;
	}

	function repaintHints() {
		var hints = document.querySelectorAll('.marker-form-hint[data-submit-verb]');
		for (var i = 0; i < hints.length; i++) {
			hints[i].replaceWith(hintNode(hints[i].getAttribute('data-submit-verb')));
		}
	}
	...
	function textareaForm(draftKey, placeholder, onSubmit, onCancel) {
		...
		// The shortcut posts, so the mouse never has to leave the keyboard
		textarea.addEventListener('keydown', function (event) {
			if (!submitsOn(event)) return;
			event.preventDefault();
			submit();
		});
	...
	function ensureSelectionButton() {
		...
		function action(className, icon, label, run, badge) {
			...
			button.addEventListener('mousedown', function (event) {
				...
				// The native selection goes before the form or editor opens, not
				// after: opening focuses a textarea, and clearing the document
				// selection after that leaves the textarea focused but takes
				// its caret with it — Chrome draws both from the same selection,
				// and a reader who sees no caret reaches for the mouse
				hideSelectionButton();
		...
		// Edit first, then Comment: the order the buttons are made is the
		// order they sit in
	...
	function openEditor(sel) {
		...
		fileContent().then(function (data) {
			...
			state.editor = {
				...
				base: slice.text,
				text: slice.text,
				// Placed under the last line selected, where a comment form
				// goes too: the panel opens below what the reader just dragged
				// over, not in the middle of it
				anchorLine: sel.lineEnd,
			...
			render();
			// Opened by the reader, so it comes into view; render's own focus
			// is the one for re-renders and keeps the page still
			focusSoon({scroll: true});
		}).catch(function (error) {
	...
	// A write located by its text rather than its line numbers, so it still
	// lands correctly after something else moved the lines. Applied writes
	// have no undo of their own: one was shipped as ↶/↷ beside the page
	// controls and taken out as clutter, the file's own history being the
	// place for that. Typing inside the textarea is the browser's own undo.
	...
	function editorPanel() {
		...
		actions.appendChild(cancel);
		actions.appendChild(hintNode('apply'));
		...
		function submit(options) {
			...
			applyEdit(entry, options).then(function () {
				closeEditor();
			}).catch(function (error) {
	...
	// Set on the index page, which is a different tab
	window.addEventListener('storage', function (event) {
		if (event.key === SUBMIT_KEY || event.key === null) repaintHints();
	});
