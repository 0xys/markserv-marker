...
(function () {
	...
	function pinRowWidgets() {
		...
		[].forEach.call(root.querySelectorAll('table'), function (table) {
			...
			var widths = widgets.map(function (widget) {
				return widget.parentElement.clientWidth || table.clientWidth;
			});
	...
	function closeEditor() {
		clearPending();
		state.editor = null;
		// Every panel, and the row any of them rode in: an emptied host row
		// still holds a column-spanning cell, and the table is laid out
		// around it
		var open = document.querySelectorAll('.marker-editor');
		for (var i = 0; i < open.length; i++) {
			var host = open[i].parentElement &&
				open[i].parentElement.closest('tr.marker-thread-row');
			open[i].remove();
