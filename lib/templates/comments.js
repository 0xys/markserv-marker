...
(function () {
	...
	function highlightQuote(thread) {
		...
		// All occurrences of the quote, as raw-text ranges
		var occurrences = [];
		var searchFrom = 0;
		var index;
		...
		// Prefer, in order: an occurrence inside the commented line range,
		// then the first one after that region starts, then the first at all
		var region = lineRangeRegion(collected.entries, thread.lineStart, thread.lineEnd);
		var chosen = null;
		if (region) {
			for (var o = 0; o < occurrences.length; o++) {
				if (occurrences[o].rawStart >= region.start && occurrences[o].rawEnd <= region.end) {
					chosen = occurrences[o];
					break;
				}
			}
			if (!chosen) {
