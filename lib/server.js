...
const md = new MarkdownIt({
	...
	// Must stay last: wraps the fence renderer installed by highlightjs
	.use(sourceLine)
...
const createHandlers = flags => {
	...
	const renderMarkdown = (reg, filePath, res) => {
		...
		return getFile(filePath).then(markdownToHTML).then(content => {
			...
			const isRegisteredFile = reg.type === 'file' && filePath === reg.path
			...
			const handlebarData = {
				...
				content,
				rootDir: reg.root,
				fileId: isRegisteredFile ? reg.id : false,
				filePath,
				fileUrl: reg.urlPath
			}
	...
	const rerenderClient = (ws, reg, rest) => {
		...
		if (!stat.isDirectory() && isType(fileTypes.markdown, filePath)) {
			...
			try {
				...
				headerHtml += '</h1>\n'
				const footerHtml = '<footer><sup><hr> Served by <a href="https://github.com/markserv/markserv">markserv</a>-marker | <a href="/">index</a> | PID: ' + process.pid + '</sup></footer>'
