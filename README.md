...
- **Step out to the folder.** The path above the page frame links the folder the file sits in, so the neighbouring documents are one click away. Folder listings carry the same path strip.
- **Keyboard, your way.** The index page's `Settings` tab chooses whether a comment posts and an edit applies on `Shift + Enter` or on plain `Enter`; the other one inserts a newline. Either way, the Enter that commits an IME conversion candidate never posts.
