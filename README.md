...
- **Review comments.** Select any text in the rendered page and a floating Comment button appears; the comment records the enclosing source-line range plus the selected text (`quote`), which stays highlighted in the page. Comments support threads and resolve/unresolve, and either a single comment or a whole thread can be deleted from the page — the thread's own button sits in its header, within reach while it is collapsed. They live in memory for the daemon's lifetime — no files written.
